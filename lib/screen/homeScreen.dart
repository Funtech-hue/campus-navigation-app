...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _updateCurrentLocationName(double latitude, double longitude) async {
    // Check if user is within North Campus (500-meter radius from 7.7115, 4.5149)
    ...
    if (distanceToCampus <= campusRadius) {
      // User is in North Campus, check for nearby Firestore location
      ...
      // User is outside North Campus, use reverse geocoding
      try {
  ...
  void _startLocationUpdates() {
    _positionStream = Geolocator.getPositionStream(
      ...
      onError: (e) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Location update error: $e')),
        );
      },
    );
  }
